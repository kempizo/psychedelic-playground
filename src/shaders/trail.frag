...
void main() {
  ...
  float orphanTrail = smoothstep(0.024, 0.20, prevLuma) * (1.0 - smoothstep(0.010, 0.14, currentLuma)) * (0.58 + trailEdgeEnergy * 0.22);
  float chromaEnergy = clamp(uAudioDetail * 0.12 + uAudioTurbulence * 0.16 + uTreblePulse * 0.08, 0.0, 1.0) * (0.14 + trailEdgeEnergy * 0.18);
  ...
  float localDecay = uDecay * mix(1.0, 0.68, edge * (0.42 + trailEdgeEnergy * 0.16));
  localDecay *= mix(1.0, 0.42, hardEdge * (0.56 + trailEdgeEnergy * 0.16));
